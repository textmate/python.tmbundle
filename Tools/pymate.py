...
def wrap(text, width):
    '''
        ...
        Written by Mike Brown, taken from the ASPN Python Cookbook
    '''
...
href="mailto:domenico.carbotta@gmail.com">the author</a> :)</small>'''
    ...
    try:
        ...
        if e_class is SystemExit:
            ...
            return
        elif e_class is SyntaxError:
            # when we get a syntax error, the traceback is not interesting
            # but we want to format differently the exception parameters
            e_args = (' in <a href="txmt://open?url=file://%s&line=%d">' +
                    '%s</a> at line %d')
            e_args = e_args % (e_obj.filename, e_obj.lineno,
                    os.path.basename(e_obj.filename), e_obj.lineno)
